...
let rec mods a b c = match a,b with
		Impl(q,w), Impl(e,r) -> if q = b || e = a then true else false
	|	Impl(q,w), _ -> if q = b then true else false
	|	_, Impl(e,r) -> if e = a then true else false
	|	_, _ -> false;;
...
let rec valid_hprooftree tree = match tree with
	...
	|	KSR(a,(Impl(p,Impl(q,r)))) -> if p = r then true else(
			match tree with
				KSR(a,(Impl(Impl(p,Impl(q,r)), Impl(Impl(v,g), Impl(n,m))))) -> if p = v && g = q && n = p && m = r then true else( 
					match tree with
						KSR(a,(Impl(Impl(p,q), Impl(Impl(r,s),t)))) -> if (p = r) && (q = Not(s) || s = Not(q)) && (p = Not(t) || t = Not(p)) then true else false
					)
				)
	|   Mod(a, b, c, d) -> match (c,d) with 
...
let rec dedthm tree = match tree with 
	...
	|	KSR(X(p)::xs, q) -> if p = q then proof_p_impl_p xs p else Mod(xs, Impl(p,q), Hyp(xs,q), KSR(xs, Impl(q,Impl(p,q))))
